...
def _path(target, pack):
    return paths.cache_dir(target, pack) / "history.jsonl"
...
def read(target, pack):
    path = _path(target, pack)
    if not path.is_file():
        return []
    out = []
...
def append(target, pack, entry):
    with _path(target, pack).open("a") as fh:
