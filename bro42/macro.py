...
def make_copy(target, pack):
    """B17: never run destructive make targets in the student's own tree."""
    dest = paths.cache_dir(target, pack) / "target-copy"
    if dest.exists():
        shutil.rmtree(dest)
    dest.mkdir(parents=True)
    for pattern in pack.tree:
...
def ensure_full_build(work, pack):
    """Leave the archive containing every function the repo has written.

    Earlier checks rebuild the tree (check_relink runs make twice), and in a
    Libft repo where Part 3 sits behind a `bonus` rule that leaves the list
    ...
    The recovery below is Libft's own legacy-naming quirk (see
    check_structure's Part 3 naming note) - a pack with one shared symbol
    (ft_printf) has nothing analogous to recover, so `make` alone is enough
    for anything else.
    """
    run(["make"], work)
    if pack.id != "libft":
        return
    part3 = [s for s, m in pack.suites.items() if m["part"] == 3]
    written = [f for f in part3 if source_of(work, f)]
    if written and any(f not in archive_symbols(work / pack.artifact[1])
                       for f in written):
        run(["make", "bonus"], work)
...
def check_structure(target, pack, out):
    root = Path(target)
    for name, why in (("Makefile", "Subject IV.1: all files at the root"),
                      (pack.header, "Subject IV: Files to Submit")):
        ok = (root / name).is_file()
        out.append(Check(1, f"{name} present", "OK" if ok else "FAIL",
                         f"{name} {'found' if ok else 'missing'} at the root",
                         hint=why))

    # A pack whose turn-in list allows arbitrary filenames (sources is None -
    # ft_printf) has no closed set to check a source exists FOR - there is no
    # basename to look for on disk. See build.py's Pack.sources comment.
    if pack.sources is not None:
        suites = pack.suite_ids
        missing = [f for f in suites if source_of(root, f) is None]
        out.append(Check(
            1, "mandatory sources", "OK" if not missing else "FAIL",
            f"{len(suites) - len(missing)}/{len(suites)} present",
        ...
        if pack.id == "libft":
            part3 = [s for s, m in pack.suites.items() if m["part"] == 3]
            legacy = [f for f in part3 if (root / f"{f}_bonus.c").is_file()]
            if legacy:
                out.append(Check(
                    1, "Part 3 naming", "OK",
        ...
        known = {f"{s}.c" for s in pack.sources}
        stray = sorted(p.name for p in root.glob("*.c") if p.name not in known)
    else:
        out.append(Check(1, "mandatory sources", "SKIP",
                         "no closed source-name set for this pack", sev="skip",
                         hint="this project's turn-in list allows arbitrary "
                              "filenames - see the build output instead"))
        stray = []
...
def check_rules(work, pack, out):
    artifact = pack.artifact[1]
    r = run(["make", "re"], work)
    built = (work / artifact).is_file()
    out.append(Check(2, "make / all", "OK" if built else "FAIL",
                     f"{artifact} built" if built else f"no {artifact} after make",
    ...
    run(["make", "clean"], work)
    left = objects(work)
    lib = (work / artifact).is_file()
    ok = not left and lib
    out.append(Check(2, "make clean", "OK" if ok else "FAIL",
                     f"objects removed, {artifact} kept" if ok else
                     f"{len(left)} .o left, {artifact} {'kept' if lib else 'DELETED'}",
                     command="make clean",
                     hint="clean removes intermediate objects only"))

    run(["make", "fclean"], work)
    ok = not objects(work) and not (work / artifact).is_file()
    out.append(Check(2, "make fclean", "OK" if ok else "FAIL",
                     f"objects and {artifact} removed" if ok else "something survived",
                     command="make fclean",
                     hint="fclean is clean plus the build product"))
    run(["make"], work)

    # The rest of this check is Libft's own legacy-naming quirk (see
    # check_structure's Part 3 naming note): a pack with one shared symbol
    # has nothing here to recover via a `bonus` rule - either `make` alone
    # produced it above, or check_symbols/check_archive will say so.
    if pack.id != "libft":
        return True
    ...
    syms = archive_symbols(work / artifact)
    ...
    out.append(Check(
        ...
        command=f"make && nm -g --defined-only {artifact}",
        hint="a rule that does nothing satisfies a grep - the symbols are the "
             ...
             f" compiled but never reached {artifact}. If they are behind a "
...
def check_relink(work, pack, out):
    artifact = pack.artifact[1]
    run(["make"], work)
    before = mtimes(work)
    lib_before = (work / artifact).stat().st_mtime_ns
    time.sleep(0.01)
    r = run(["make"], work)
    after = mtimes(work)
    moved = [k for k in after if before.get(k) != after[k]]
    relinked = (work / artifact).stat().st_mtime_ns != lib_before
    ...
    out.append(Check(
        ...
        + (f" and {artifact} was re-created" if relinked else ""),
    ...
    if header.is_file():
        ...
        out.append(Check(
            ...
            f"touching {pack.header} recompiled {len(moved)} object(s)",
            sev="warn", command=f"touch {pack.header} && make",
...
def check_prototypes(target, work, pack, out):
    """Do the sources match the prototypes Subject IV.2 specifies?

    Libft-only (called from audit() only when pack.id == "libft" - see
    _dev/plan/platform/10-packs.md's argument against generalising ahead of
    evidence): this is invisible from inside the project. A student whose
    libft.h declares `int ft_memcmp(void *s1, ...)` - no const - compiles and
    links and runs perfectly, because their own header agrees with their own
    source and C does not check parameter types across translation units. It
    breaks the moment anyone includes the subject's prototype instead, which
    is what an evaluator's main does.
    ...
    sources = [p for p in Path(target).glob("*.c")
               if p.stem in pack.sources or p.stem.replace("_bonus", "") in pack.sources]
    if not sources:
        return
    for src in sources:
        shutil.copy2(src, probe / src.name)
    build.synth_header(pack, probe)

    from . import content
    protos = {n: m.get("prototype", "")
              for n, m in content.load(pack)["functions"].items()}
...
def check_symbols(work, pack, out):
    ensure_full_build(work, pack)
    artifact = pack.artifact[1]
    archive = work / artifact
    if not archive.is_file():
        out.append(Check(5, "symbol audit", "SKIP", f"no {artifact} to inspect",
                         sev="skip"))
        return
    ...
    out.append(Check(
        ...
        command=f"nm -g --defined-only {artifact}",
    ...
    obj = None
    offenders = {}
    # A call to a symbol the PACK ITSELF supplies is legal - it is the
    # student's own code, just not (necessarily) one of its named suites.
    own_names = set(pack.symbols) | {f"{s}_bonus" for s in pack.symbols}
    for line in r.stdout.splitlines():
        line = line.strip()
        if line.endswith(".o:"):
            obj = line[:-1]
        ...
        part = pack.part_of(fn)
        if sym not in pack.allowed_externals[part]:
...
def check_archive(work, pack, out):
    ...
    out.append(Check(6, "valid ar archive", "OK" if ok else "FAIL",
                     ...
                     command=f"file {artifact}",
                     hint="Subject IV.1: you must use ar to create your library"))

    syms = archive_symbols(archive)
    # A pack with a closed source set can ask "does every symbol whose
    # source EXISTS also appear in the archive" - the same question
    # check_structure's "mandatory sources" asks, one step later in the
    # pipeline. A pack that cannot enumerate its sources (ft_printf) can
    # only ask it of the symbols it actually defines.
    if pack.sources is not None:
        expected = [f for f in pack.suite_ids if source_of(work, f)]
    else:
        expected = sorted(pack.symbols)
...
def check_readme(target, pack, out):
    ...
    out.append(Check(8, "first line", "OK" if ok else "FAIL",
                     ...
                     hint="Subject V: the first line must be italicised and read "
                          ...
                          "curriculum by <login>.*"))
    for name in pack.readme_sections:
...
class Finding:
    ...
    def as_check(self, pack):
        card = content.concept(pack, self.concept)
...
def scan_pitfalls(work, pack):
    """Libft-only (see PITFALL_SCANS' own module comment and audit() below) -
    every scan here names a Libft function or file outright."""
    out = []
    for path in sorted(Path(work).glob("*.c")):
        text = _strip_comments(path.read_text(errors="replace"))
        for scan in PITFALL_SCANS:
            out += [f.as_check(pack) for f in scan(path, text)]
    return out


# ---------------------------------------------------------------- driver

def audit(target, pack):
    out = []
    check_structure(target, pack, out)
    work = make_copy(target, pack)
    if (work / "Makefile").is_file():
        if check_rules(work, pack, out):
            check_relink(work, pack, out)
            check_flags(work, out)
            # Libft-only - _dev/plan/platform/10-packs.md argues against
            # generalising ahead of evidence, and both check_prototypes and
            # the pitfalls scanner below are written entirely in terms of
            # Libft's own sources.
            if pack.id == "libft":
                check_prototypes(target, work, pack, out)
            check_symbols(work, pack, out)
            check_archive(work, pack, out)
    else:
        out.append(Check(2, "Makefile rules", "SKIP", "no Makefile to run",
                         sev="skip"))
    check_norm(work, out)
    check_readme(target, pack, out)
    if pack.id == "libft":
        out += scan_pitfalls(work, pack)
    return out
