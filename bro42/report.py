...
# Schema 2 (Phase 2, packs): adds the top-level "pack" block below, so the
# dashboard can stop knowing Libft. Nothing else about the shape of an
...
def build_report(records, target, pack, checks=None, hist=None, hist_delta=None):
    cases = [r for r in records if r.get("kind") != "macro"]
    scored = [r for r in cases if r["status"] not in UNSCORED]
    funcs = content.load(pack)["functions"]
    ...
    return {
        ...
        "target": str(target),
        "bro_version": VERSION,
        # The dashboard's only source for project-specific labels (part
        # names, level names, macro section names) - so it never has to know
        # Libft exists, the same way this Python side no longer does.
        "pack": {
            "id": pack.id,
            "display": pack.display,
            "rank": pack.rank,
            "parts": {str(k): v for k, v in pack.parts.items()},
            "levels": {str(k): v for k, v in pack.levels.items()},
            "macro_sections": {str(k): v for k, v in pack.macro_sections.items()},
        },
        ...
        "delta": hist_delta,
        "history": hist or [],
        "concepts": content.concepts(pack),
        "defense": content.defense_bank(pack),
        "macro": checks or [],
...
def write(records, target, pack, checks=None, hist_delta=None):
    cache = paths.cache_dir(target, pack)
    hist = history.read(target, pack)[-60:]
    report = build_report(records, target, pack, checks, hist, hist_delta)
