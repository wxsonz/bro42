...
/*
** Thin wrapper over bro_expect_bytes for NUL-terminated strings: compares
** through the SHORTER string's terminator (min(strlen) + 1 bytes). That is
** enough to catch every case -
**   - equal length: the full comparison, including both NULs
**   - unequal length: the shorter string's NUL lines up against a live byte
**     in the longer one at index min(strlen), which is itself the first
**     divergence
** and it never reads past either string's own NUL, which a naive
** max(strlen) window could do to whichever string is shorter.
*/
void	bro_expect_str(t_result *r, const char *expected, const char *actual)
{
	...
	alen = strlen(actual);
	n = elen < alen ? elen : alen;
	bro_expect_bytes(r, expected, actual, n + 1);
}
