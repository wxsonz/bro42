...
/*
** The per-pack "before each case" hook. Every pack defines it; the core
** only promises to call it once per case, before the case runs.
**
** This replaces a forward declaration of a Libft-only symbol that the
** core/pack split left behind as a documented leak. It stopped being
** merely untidy the moment a second pack existed: that symbol lives in
** Libft's list.c, so linking any other pack failed with an undefined
** reference. A hook the core names and each pack fills is what the leak
