...
from . import build, content, history, macro, micro, render, report

PART_OF_SELECTOR = {"part1": 1, "part2": 2, "part3": 3}


def parse_args(argv):
    p = argparse.ArgumentParser(prog="bro", add_help=True,
                                description="a Libft gym bro")
    p.add_argument("selector", nargs="?", default=None,
                   help="function or fn:id, e.g. ft_memmove or ft_memmove:03")
    p.add_argument("--target", default=None, help="path to the libft repository")
...
def main(argv=None):
    ...
    if args.version:
        print(f"ft_bro {__import__('ft_bro').VERSION}")
        return 0
    if args.check_update:
        return update.check()
    if args.where:
        print(paths.cache_dir(target))
        return 0
    if args.clean:
        return cmd_clean(target)
    if args.serve:
        from . import serve as serve_mod
        return serve_mod.main(target)
    ...
    if argv and argv[0] == "explain":
        ...
        if binary:
            recs, _ = micro.run(binary, skip=info.get("missing"))
        print(render.concept_card(slug, st_early(args), recs))
        return 0
    if argv and argv[0] == "defense":
        fn = argv[1] if len(argv) > 1 and not argv[1].startswith("-") else None
        print(render.defense_cards(st_early(args), args.concept, fn))
        return 0
    ...
    if mode == "macro":
        ...
        if args.json:
            ...
            return 0
        print(f"\n  {st.bold('ft_bro')}  {st.dim(str(target))}\n")
        print(render.macro_line(checks, st))
        print()
    ...
    if argv and argv[0] == "next":
        return cmd_next(target, st, binary, info)
    if argv and argv[0] == "debug":
        return cmd_debug(target, st, binary, argv[1:])
    ...
    fn, cid = normalise_selector(args.selector)
    only = f"{fn}:{cid}" if fn and cid else (fn if fn else None)
    records, err = micro.run(binary, only=only, timeout_ms=args.timeout,
    ...
    page = None
    if not restricted:
        page = report.write(records, target, checks, hist_delta)
...
def cmd_next(target, st, binary, info):
    ...
    for fn, meta in ordered:
        ...
        slugs = sorted({k for c in content.load()["cases"].values()
                        if c["fn"] == fn for k in c["kw"]})
        for slug in slugs:
            print(render.concept_card(slug, st))
        return 0
    ...
    records, _ = micro.run(binary, skip=info.get("missing"))
